...
## Слой Model
 ...
 Класс  **Model** (абстрактный) - публикует событие об изменении модели, чтобы все подписчики обновились.

 ### Файл AppData.ts
 #### 🎸 class ItemData
  расширяет класс **Model**, имеет интерфейс **IItemData** (описывает данные товара). Имеет поля:
`id?: string;`
  `description?: string`;
`image?: string;`
  `title: string;`
